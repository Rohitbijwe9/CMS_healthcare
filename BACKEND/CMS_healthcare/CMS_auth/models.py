...
class User(AbstractUser):
    ...
    user_code = models.CharField(max_length=20,unique=False)
    user_image = models.ImageField(upload_to='user/',blank=True)
    is_active = models.BooleanField(default=True)
    created_on = models.DateTimeField(auto_now_add=True,editable=False)
    #contact_details = models.OneToOneField(ContactDetails,on_delete=models.CASCADE,related_name='contact_details') # delete
    #service_provider = models.ForeignKey(ServiceProvider,on_delete=models.CASCADE,related_name='service_provider') # delete
